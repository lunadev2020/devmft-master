    ...
    m_queue(nullptr)
{
    ...
    try {
        ...
        m_Skill = skillDescriptor.CreateSkillAsync().get().as<ObjectDetectorSkill>();

        // Create instance of the skill binding
        m_Binding = m_Skill.CreateSkillBindingAsync().get().as<ObjectDetectorBinding>();

        DMFTRACE(DMFT_GENERAL, TRACE_LEVEL_INFORMATION, "%!FUNC! CREATE WIN AI SKILL SUCCESS!!!");
    }
    catch (winrt::hresult_error const& ex)
    {
        std::wstring wbuf = ex.message().c_str();
        size_t wleng = wbuf.length();
        ...
        DMFTRACE(DMFT_GENERAL, TRACE_LEVEL_INFORMATION, "%!FUNC! CREATE WIN AI SKILL ERROR: %s", buf);
    }
#endif
